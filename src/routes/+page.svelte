<script>
	...
	let referralToken = $state(null);
	let referralInfo = $state(null);

	// Handle referral tokens and initial setup
	onMount(() => {
		...
		if (refToken) {
			...
			validateReferral(refToken);
		}
	});
	
	async function validateReferral(token) {
		try {
	...
	async function ask() {
		...
		try {
			...
			// Include referral token in first message
			if (referralToken && messages.length <= 1) {
				requestBody.referral_token = referralToken;
			}
			...
			if (data.trust) trust = data.trust;
...
<div class="min-h-screen bg-white">
	<header class="border-b border-gray-100">
		<div class="max-w-2xl mx-auto px-6 py-4 flex items-center justify-between">
			...
			<div class="flex items-center gap-4">
				...
				<!-- Trust Badge -->
				{#if trust && messages.length > 0}
					<div class="text-xs px-2 py-1 rounded-full font-medium
						{trust.tier === 'new' ? 'bg-gray-100 text-gray-600' : ''}
						{trust.tier === 'known' ? 'bg-blue-100 text-blue-700' : ''}
						{trust.tier === 'trusted' ? 'bg-green-100 text-green-700' : ''}
						{trust.tier === 'vip' ? 'bg-purple-100 text-purple-700' : ''}
					">
						{#if trust.tier === 'new'}👋{:else if trust.tier === 'known'}⭐{:else if trust.tier === 'trusted'}✨{:else if trust.tier === 'vip'}👑{/if}
						{trust.tier.charAt(0).toUpperCase() + trust.tier.slice(1)}
					</div>
				{/if}
	...
	<div class="max-w-2xl mx-auto px-6">
		...
		<!-- Recent activity (subtle social proof) -->
		{#if data.recentActivity && data.recentActivity.length > 0}
			<div class="py-8">
				<p class="text-xs font-medium text-gray-400 uppercase tracking-widest mb-4 text-center">Recent activity</p>
				<div class="space-y-2">
					{#each data.recentActivity as event}
						<div class="bg-gray-50 rounded-lg px-3 py-2 text-xs text-gray-600 text-center">
							{event.display_text}
							<span class="text-gray-400 ml-2">{new Date(event.created_at).toLocaleDateString()}</span>
						</div>
					{/each}
				</div>
				<div class="text-center mt-3">
					<a href="/wall" class="text-xs text-gray-500 hover:text-gray-700 underline">View all activity</a>
				</div>
			</div>
		{/if}
